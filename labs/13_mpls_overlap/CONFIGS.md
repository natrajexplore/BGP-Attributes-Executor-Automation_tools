# 13_mpls_overlap: all device configurations

MPLS L3VPN, two customers with the same 10.1.0.0/24, and a route-target leak
...
1. Import and start the lab (`labs/labtool.sh 13_mpls_overlap import` and `start`, or import the `.zip` in the EVE web UI and start all nodes).
...
|---|---|---|---|---|
| CE-A1 | e1/0 | PE1 | e1/1 | 172.16.1.0/30 eBGP PE-CE, customer A site 1 |
| CE-B1 | e1/0 | PE1 | e1/2 | 172.16.2.0/30 eBGP PE-CE, customer B site 1 |
| PE1 | e1/0 | P | e1/0 | 10.0.1.0/30 core (OSPF, LDP) |
| P | e1/1 | PE2 | e1/0 | 10.0.2.0/30 core (OSPF, LDP) |
| PE2 | e1/1 | CE-A2 | e1/0 | 172.16.3.0/30 eBGP PE-CE, customer A site 2 |
| PE2 | e1/2 | CE-B2 | e1/0 | 172.16.4.0/30 eBGP PE-CE, customer B site 2 |
...
!
interface Ethernet1/1
 description === PE-CE to CE-A1, VRF CUST-A ===
...
interface Ethernet1/2
 description === PE-CE to CE-B1, VRF CUST-B ===
...
!
interface Ethernet1/1
 description === PE-CE to CE-A2, VRF CUST-A ===
 ip vrf forwarding CUST-A
...
!
interface Ethernet1/2
 description === PE-CE to CE-B2, VRF CUST-B ===
 ip vrf forwarding CUST-B
...
router bgp 65000
 ...
 exit-address-family
 address-family ipv4 vrf CUST-A
  neighbor 172.16.3.2 remote-as 65102
  neighbor 172.16.3.2 activate
 exit-address-family
 address-family ipv4 vrf CUST-B
  neighbor 172.16.4.2 remote-as 65202
...
### Scenario `13_mpls_overlap`: L3VPN - one extra export route-target leaks customer B into customer A

Customer B's second site (10.3.0.0/24) is exported by PE2 with route-target 65000:200, which only VRF CUST-B imports. The scenario adds customer A's route-target 65000:100 as a second export on VRF CUST-B at PE2. PE1's VRF CUST-A now imports 10.3.0.0/24 and customer A's router learns a network that belongs to customer B. Rollback removes the extra export.
