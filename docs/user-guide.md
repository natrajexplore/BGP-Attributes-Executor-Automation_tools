...
6. To look at a router yourself, use the **EVE-NG** card below: **CLI tab** selects the router in the SSH / CLI panel and shows its BGP summary, and you can type any read-only `show` command there. **SSH session** opens the router in its own PuTTY window. Set it up once on your PC by running `powershell -ExecutionPolicy Bypass -File scripts\putty-setup.ps1` in the repository folder (see [`live-labs.md`](live-labs.md)); allow the browser's prompt to open PuTTY links, click **Accept** on PuTTY's host-key alert the first time, and log in as `lab`. Router login and enable passwords are on the **Credentials** tab (hidden until you click Reveal).
